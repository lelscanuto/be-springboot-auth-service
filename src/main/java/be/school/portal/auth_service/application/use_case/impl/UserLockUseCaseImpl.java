...
import be.school.portal.auth_service.application.mappers.UserProjectMapper;
...
public class UserLockUseCaseImpl implements UserLockUseCase {
   ...
   * @param userProjectMapper the mapper for converting user entities to lightweight DTOs
   */
  public UserLockUseCaseImpl(UserRepository userRepository, UserProjectMapper userProjectMapper) {
   ...
   * @return a {@link CompletableFuture} containing a {@link UserLiteDTO} representation of the
   *     locked user
   * @throws UserNotFoundException if no user with the given username exists
   */
  @Override
  @Async
  public CompletableFuture<UserLiteDTO> lockUser(String username) {
    ...
    return CompletableFuture.completedFuture(userProjectMapper.toUserLiteDTO(existingUser));
  }
}
